...
local background_image = Image.ScaleFill(
...
local color_separator = Color.Create(0.8, 0.8, 0.8)


local titlebar_separator = Image.Extent(Point.Create(resolution_x, 1), color_separator)
...
titlebar = Image.Group({
	...
	Image.Translate(
		titlebar_separator,
		Point.Create(0, titlebar_container_height - 5)
	),
	Image.Translate(
		titlebar_separator,
		Point.Create(0, titlebar_container_height - 1)
	)
})
...
local sidebar_inner_width = sidebar_width - 5
local sidebar_inner_size = Point.Create(sidebar_inner_width, sidebar_height)
...
function create_sidebar_buttons_container()
	...
	local button_container_separator = Image.Extent(Point.Create(sidebar_inner_width, 1), color_separator)

	return Image.Group({
		Image.Translate(
			button_container_separator,
			Point.Create(0, 0)
		),
		Image.Translate(
			button_container_separator,
		...
		Image.Extent(Point.Create(sidebar_width - 20, sidebar_button_container_height), Color.Create(0, 0, 1, 0.0)),
...
function create_sections_container()
	...
	local button_background = Image.Extent(button_size, Color.Create(0, 0, 1, 0.2))
	local button_background_selected = Image.Extent(button_size, Color.Create(0, 0.5, 1, 0.5))
...
local vertical_separator = Image.Extent(Point.Create(1, sidebar_height), color_separator)

local sidebar = Image.Group({
	-- Image.Extent(sidebar_size, Color.Create(0, 0, 0.0)),
	Image.Translate(sidebar_sections_container, Point.Create(0, 10)),
	Image.Justify(sidebar_buttons_container, sidebar_inner_size, Justify.Bottom),
	Image.Translate(
		vertical_separator,
		Point.Create(sidebar_width-5, 0)
	),
	Image.Translate(
		vertical_separator,
		Point.Create(sidebar_width-1, 0)
	)
})
...
function create_contents_container()
	...
	for key, config_func in pairs(section_entries) do
		lazy_sections[key] = Image.Lazy(function ()
			...
			return Global.create_vertical_scrolling_container(content_image, content_container_size)
		end)
	end
